...
void load_power(float *pIn, float local_pIn[TILE_Z][TILE_Y][TILE_X], int tileOffset) {
    LOAD_POWER_LOOP:
...
// load in with halo
void load_in_with_halo(float *in, float local_in[TILE_Z][TILE_Y + 2][TILE_X], int tileOffset, TileStatus isBoundary) {
...
void load(float *pIn, float *tIn, float local_pIn[TILE_Z][TILE_Y][TILE_X], float local_tIn[TILE_Z][TILE_Y + 2][TILE_X], int tileOffset, TileStatus isBoundary) {
...
// Store function
void store(float* tOut, float local_tOut[TILE_Z][TILE_Y][TILE_X], int tileOffset) {
    #pragma HLS inline off
    STORE_LOOP:
    for (int z = 0; z < TILE_Z; z++) {
        for (int y = 0; y < TILE_Y; y++) {
            for (int x = 0; x < TILE_X; x++) {
                int localIdx = y * TILE_X + z * TILE_X * TILE_Y + x;
                int globalIdx = tileOffset + (y * NX + z * NX * NY) + x;                
                tOut[globalIdx] = local_tOut[z][y][x];   
            }
        }
    }
}
